...
if ( ! function_exists( 'museum_comment' ) ) :
/**
 * Template for comments and pingbacks.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 */
function museum_comment( $comment, $args, $depth ) {
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
		<div class="comment-body">
			<?php _e( 'Pingback:', 'museum' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'museum' ), '<span class="edit-link">', '</span>' ); ?>
		</div>
				...
				<div class="comment-metadata">
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
						<time datetime="<?php comment_time( 'c' ); ?>">
							<?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'museum' ), get_comment_date(), get_comment_time() ); ?>
						</time>
					</a>
...
endif; // ends check for museum_comment()
